...
-- coroutines
local gen = coroutine.create(function(a, b)
  show("co start", a, b, coroutine.isyieldable(), select(2, coroutine.running()))
  local c = coroutine.yield(a + b)
  local d, e = coroutine.yield(c * 2)
  return d + e, "done"
end)
show("co type", type(gen), coroutine.status(gen), coroutine.isyieldable())
show("co resume 1", coroutine.resume(gen, 1, 2))
show("co status", coroutine.status(gen))
show("co resume 2", coroutine.resume(gen, 10))
show("co resume 3", coroutine.resume(gen, 3, 4))
show("co dead", coroutine.status(gen), coroutine.resume(gen))
local failing = coroutine.create(function() local x = nil; return x.field end)
show("co error", coroutine.resume(failing))
show("co error status", coroutine.status(failing))
local nested = coroutine.wrap(function()
  local inner = coroutine.wrap(function() coroutine.yield("inner 1"); return "inner 2" end)
  coroutine.yield(inner())
  coroutine.yield(inner())
  return "outer done"
end)
show("co wrap", nested(), nested(), nested())
try("co wrap dead", nested)
try("co wrap error", coroutine.wrap(function() error("boom") end))
local inpcall = coroutine.wrap(function()
  local ok, v = pcall(function() return coroutine.yield("from pcall") .. "!" end)
  return tostring(ok) .. " " .. v
end)
show("co yield in pcall", inpcall(), inpcall("back"))
try("co yield outside", coroutine.yield, 1)
local selfref
selfref = coroutine.create(function()
  show("co running", coroutine.running() == selfref, coroutine.status(selfref))
  try("co resume running", coroutine.resume, selfref)
end)
coroutine.resume(selfref)
local closing = coroutine.create(function() coroutine.yield(1) end)
coroutine.resume(closing)
show("co close", coroutine.close(closing), coroutine.status(closing))
try("co resume bad", coroutine.resume, 42)
local producer = coroutine.wrap(function()
  for i = 1, 3 do coroutine.yield(i) end
end)
local total = 0
for _ = 1, 3 do total = total + producer() end
show("co producer", total)
