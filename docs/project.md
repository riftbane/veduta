...
| `entry` | string | `"./cmd/game"` | Go package of the game binary, as given to `go build`: `"."` or a relative path starting with `./`. Not used, and not allowed, in a script game. |
| `script` | string | none | The main Lua file of a script game, relative to the project root (`"main.lua"`). Set, the game is Lua code the tool runs itself: no Go, no build; the `lua` topic is its API. Since v2.0.0. |
| `api` | integer | `1` | The Lua API level a script game needs. A runtime whose level is lower refuses the game with a message naming both, instead of failing on a missing function in the middle of play. Only for a script game. Since v2.0.0. |
