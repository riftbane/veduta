...
| `SCENE_ZFIGHT_RISK` | warning | Faces of two different entities lie in the same plane (within 1 mm, normals within about 0.8°), face the same way (or one is double-sided) and overlap by more than 0.1% of the smaller triangle. Two faces that are both `"alpha": "blend"` do not count, because neither writes depth and the draw order decides. | Move the smaller entity at least 0.01 m along the normal (`entities[i].position`), remove the covered face, or set `cull` to `"back"`. |
