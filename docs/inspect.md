...
| `SCENE_OVERLAP` | warning | The AABBs of two `static` entities intersect by more than 1 mm on every axis. Touching boxes, parent/descendant pairs and pairs whose models are both drawn only with `"alpha": "blend"` materials (translucent sprites stacked by `layer`) do not count. | Move `b` (the later entity) as the hint says (`entities[i].position`): by the shortest distance along one axis that leaves the boxes only touching, rounded up to the cm, which is more than the overlap depth when one box contains the other on that axis. Or move or scale either. |
...
| `SCENE_ENTITY_OFFSCREEN` | warning | An entity tagged `important` has no visible pixel. Translucent (`"alpha": "blend"`) parts never count as occluders: an entity without a pixel in the render above is rendered again with only opaque and cutout parts, and its own parts, writing ids, and is not reported when it owns a pixel there (seen through water or mist). `reason`: `occluded` (opaque or cutout occluders listed), `outside_view`, `no_pixels`, `hidden`, `no_model`, `missing_model`, `not_measured` (after 16 measured entities). | `camera.position` / `camera.look_at`, the occluder's or the entity's `position`, `visible`, `scale`, or its `tags`. |
...
| `SCENE_ZFIGHT_RISK` | warning | Faces of two different entities lie in the same plane (within 1 mm, normals within about 0.8°), face the same way (or one is double-sided) and overlap by more than 0.1% of the smaller triangle. Faces are those of the models as drawn (a `hitbox` does not change them); two faces that are both `"alpha": "blend"` do not count, because neither writes depth and the draw order decides. | Move the smaller entity at least 0.01 m along the normal (`entities[i].position`), remove the covered face, or set `cull` to `"back"`. |
